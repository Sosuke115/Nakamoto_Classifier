...
class VGG11:
    """
        ...
        conv - relu -conv - relu - pool
        conv - relu -conv - relu - pool
        affine - relu - affine - relu -dropout -affine -dropout- softmax
        0,3,6,8,11,13,16,18,21,23,26
    ...
    def __init__(self, input_dim=(3, 224, 224),
        ...
        pre_node_nums = np.array([input_dim[0]*3*3, 64*3*3, 128*3*3, 256*3*3, 256*3*3, 512*3*3, 512*3*3, 512*3*3, 512*3*3,4096,hidden_size])
        ...
        self.params['W9'] = weight_init_scales[8] * np.random.randn(4608, 4096)
        self.params['b9'] = np.zeros(4096)
        self.params['W10'] = weight_init_scales[9] * np.random.randn(4096, hidden_size)
        self.params['b10'] = np.zeros(hidden_size)
        self.params['W11'] = weight_init_scales[10] * np.random.randn(hidden_size, output_size)
        ...
        self.layers.append(Affine(self.params['W11'], self.params['b11']))
        self.layers.append(Dropout(0.5))
    ...
    def gradient(self, x, t):
        ...
        # 設定
        grads = {}
        for i, layer_idx in enumerate((0,3,6,8,11,13,16,18,21,23,26)):
